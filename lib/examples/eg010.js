...
eg010.createController = async (req, res) => {
    let body = req.body
      ...
      , envelopeArgs = {
            ...
            ccEmail: ccEmail, 
            ccName: ccName }
      , envelopeJSON = makeEnvelopeJSON( envelopeArgs )
      , accountId = req.dsAuthCodeGrant.getAccountId()
      , basePath = req.dsAuthCodeGrant.getBasePath()
      ;
    ...
    let options = {
            method: 'POST',
            uri: `${basePath}/v2/accounts/${accountId}/envelopes`,
            auth: {bearer: req.dsAuthCodeGrant.getAccessToken()},
            headers: {
    ...
    req.session.envelopeId = envelopeId; // Save for use by other examples which need an envelopeId

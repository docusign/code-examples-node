 ...
 */
const phoneAuthentication = async (args) => {
  ...
  let results = await accountsApi.getAccountIdentityVerification(
    args.accountId, (error, data, response) => {
  ...
  results.data.identityVerification.forEach(workflow => {
    if (workflow.defaultName === 'Phone Authentication') {
...
module.exports = { phoneAuthentication };

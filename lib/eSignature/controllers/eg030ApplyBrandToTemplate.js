...
const applyBrandToTemplate = require('../examples/applyBrandToTemplate');
...
eg030ApplyBrandToTemplate.createController = async (req, res) => {
    ...
    try {
        ...
        results = await applyBrandToTemplate.createEnvelope(args);
    }
    catch (error) {
...
eg030ApplyBrandToTemplate.getController = async (req, res) => {
    ...
    if (tokenOK) {
        ...
        const results = await applyBrandToTemplate.getBrandsAndTemplates(args);

...
const exportDataFromRoom = require('../examples/exportDataFromRoom');
const validator = require('validator');
const dsConfig = require('../../../config/index.js').config;

const eg003ExportDataFromRoom = exports;
const eg = 'eg003rooms'; // This example reference.
...
eg003ExportDataFromRoom.createController = async (req, res) => {
    ...
    let roomId = validator.escape(body.roomId)
    const roomsArgs = {
        roomId
    };
    const args = {
    ...
    try {
        ...
        results = await exportDataFromRoom.getRoomData(args);
    }
    catch (error) {
...
eg003ExportDataFromRoom.getController = async (req, res) => {
    ...
    if (tokenOK) {
        ...
        const userRooms = await exportDataFromRoom.getRooms(args);

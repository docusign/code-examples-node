...
eg008.createController = async (req, res) => {
    let body = req.body
      ...
      , listTemplateP = req.dsAuthCodeGrant.makePromise(templatesApi, 'listTemplates')
      ;
    ...
    // Step 2. See if the template already exists
    let results = null;
    try {
        results = await listTemplateP(accountId, {searchText: templateName});
    } 
    catch (error) {
    ...
    let templateNameReturned = results.name;

    res.render('pages/example_done', {
        title: "Template created",
        h1: "Template created",
        message: `The template has been created!<br/>Template name: ${templateNameReturned}, ID ${templateId}.`
    });
}

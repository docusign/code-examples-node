...
eg004AddingFormToRoom.createController = async (req, res) => {
    ...
    if (results) {
        res.render('pages/example_done', {
            ...
            message: `Results from the Rooms: AddFormToRoom method. RoomId: ${results.roomId}, FormId: ${results.formId}.`,
...
eg004AddingFormToRoom.worker = async (args) => {
    ...
    let roomsApi = new docusignRooms.RoomsApi(dsApiClient)
        ...
        console.log(args.roomsArgs.formId)
    results = roomsApi.addFormToRoom(/*args.roomsArgs.formForAdd*/null, args.roomsArgs.roomId, args.accountId, null);
...
eg004AddingFormToRoom.getController = async (req, res) => {
    ...
    if (tokenOK) {
        ...
        formLibraries = await formLibrariesApi.getFormLibraries(req.session.accountId, {count: 5}, null);
        console.log('lllllllllllllll', formLibraries)
        const forms = await formLibrariesApi.getFormLibraryForms(req.session.accountId, formLibraries[0]);

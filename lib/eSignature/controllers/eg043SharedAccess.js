...
const { API_TYPES, getItem, setItem } = require('../../utils.js');
...
eg043SharedAccess.createController = async (req, res) => {
  ...
  if (results) {
    ...
    setItem(res.locals.localStorage, 'agentUserId', agentUserId);
...
eg043SharedAccess.createAuthorizationController = async (req, res) => {
  ...
  const agentUserId = getItem(res.locals.localStorage, 'agentUserId');
  const args = {
    accessToken: req.user.accessToken,
    basePath: req.session.basePath,
    accountId: req.session.accountId,
  ...
  setItem(res.locals.localStorage, 'principalUserId', user.sub);
...
eg043SharedAccess.listEnvelopes = async (req, res) => {
  ...
  const principalUserId = getItem(res.locals.localStorage, 'principalUserId');
  const args = {
    accessToken: req.user.accessToken,
    basePath: req.session.basePath,
    accountId: req.session.accountId,
